...
def plot_temperature_by_radius(nframes, z_positions, temp_type='Te', temp_factor=1.0):
...
def plot_temperature_section(nframes, temp_type='Ti', temp_factor=1.0, figsizex=18.0, figsizey=8.0):
...
def plot_temperature_cross_section_fixed_z(nframes, z_pos, num_theta=100, temp_type='Te', temp_factor=1.0):
    temp_data = collect(temp_type, yguards=True)[:, 2:-2, 2:-2, 0]
    temp_140us = temp_data[nframes] * temp_factor
    nx, ny = temp_140us.shape
    r = np.linspace(0, Rcell, nx)
...
def plot_temperature_sections_from_file(nframes, z_positions, temp_type='Ti', temp_factor=1.0):
    temp_data = collect(temp_type, yguards=True)[:, 2:-2, 2:-2, 0]
    temp_140us = temp_data[nframes] * temp_factor
    nx, ny = temp_140us.shape
    r = np.linspace(0, Rcell, nx)
